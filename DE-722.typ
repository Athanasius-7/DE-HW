...
$mat(1, 0, 1, 0; -1, 1, 0, 2; 0, -1, 25, 0; augment: #3)#tag[(arrow)]$
#sym.arrow
RREF:
$mat(1, 0, 0, -frac(1, 13); 0, 1, 0, frac(25, 13); 0, 0, 1, frac(1, 13); augment: #3)$ \
Giving us: \
...
$A + B = 1 \ 4A + B = 5$ \
Matrix of system is:
$mat(1, 1, 1; -4, -1, 5; augment: #2)#tag([arrow])$
#sym.arrow RREF
$mat(1, 0, frac(4, 3); 0, 1, -frac(1, 3); augment: #2)$\
Which gives us:\
...
#set align(center)
Setup $cal(L)$, since y(0) = y'(0) = 0, I will skip the steps involving them: \
$ s^2 Y(s) + 9Y(s) = frac(1, (s -1)) $ \
Factor LHS and islate $Y(s)$: \
$Y(s) = frac(1, (s-1)(s^2+9))$ \
$(s^2 + 9)$ is non-reducible therefore our decomposed fraction will be of the following form: \
$frac(1,(s-1)(s^2+9)) = frac(A s + B, (s^2 + 9)) + frac(C, (s-1))$ \ 
Adding both fractions: \
$frac(A s^2 - A s + B s - B + C s^2 + 9C, (s^2+9)(s-1))$
Augmented matrix is: \
$mat(1,0,1,0;-1,1,0,0;0,-1,9,1; augment: #3) #tag[(arrow)]$
#sym.arrow
RREF:
$mat(1,0,0,-frac(1,10);0,1,0,-frac(1,10);0,0,1,frac(1,10); augment: #3)$ \
Substituting and simplifying our fractions: \
$frac(-s - 1, 10(s^2 + 9)) + frac(1, 10(s-1))$ \
Getting into form for inverse Laplace: \
$Y(s) = -frac(s, 10(s^2 + 9)) - frac(1, 10(s^2 + 9)) + frac(1, 10(s-1))$ \
Which gives us our final answer of: \
*$[y = -frac(1,10)cos(3t) - frac(1,30)sin(3t) + frac(1,10)e^(t)]$* 
